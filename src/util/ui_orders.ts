...
export const ordersToIEOUIOrders = (orders: SignedOrder[], baseToken: Token, ordersInfo?: OrderInfo[]): UIOrder[] => {
    if (ordersInfo) {
        return ordersToIEOUIOrdersWithOrdersInfo(orders, ordersInfo, baseToken);
    } else {
        return [];
    }
...
// IEO orders
const ordersToIEOUIOrdersWithOrdersInfo = (
    orders: SignedOrder[],
    ordersInfo: OrderInfo[],
    baseToken: Token,
): UIOrder[] => {
